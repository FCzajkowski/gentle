...
Im sorry, for what I did. Only God knows what happens here
Once Again Im sorry
...
class Parser:
  ...
  def comp_expr(self):
    ...
    if res.error:
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        "Expected int, float, identifier, '+', '-', '(', '[', 'if', 'for', 'while', 'function', 'not'"
      ))
  ...
  def if_expr(self):
    ...
    while self.current_tok.matches(TT_KEYWORD, 'else if'):
      res.register_advancement()
      self.advance()
